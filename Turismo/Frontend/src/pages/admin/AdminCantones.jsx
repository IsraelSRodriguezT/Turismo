...
export default function AdminCantones() {
  ...
  const cargarDatos = async () => {
    ...
    setError(null);
    try {
      const [respCantones, respProvincias] = await Promise.all([
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await api.post('/geolocalizacion/cantones/', { nombre, provincia_id: Number(provincia) });
      setNombre('');
      setProvincia('');
      setShowForm(false);
      cargarDatos();
    } catch {
      alert('Error al guardar el cantón. Verifique que no esté duplicado.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleEliminar = async (id, nombreC) => {
    if (!confirm(`¿Eliminar el cantón "${nombreC}"?`)) return;
    try {
      await api.delete(`/geolocalizacion/cantones/${id}/`);
      cargarDatos();
    } catch {
      alert('No se pudo eliminar el cantón.');
    }
  };
  ...
  return (
    <>
      <nav className="flex items-center gap-xs text-on-surface-variant mb-lg font-label-md text-label-md">
        ...
        <Link to="/admin/cantones" onClick={() => setShowForm(false)} className="hover:text-primary">Gestión de Cantones</Link>
        {showForm && (<><span className="material-symbols-outlined text-[14px]">chevron_right</span><span className="font-bold text-on-surface">Nuevo Cantón</span></>)}
      </nav>

      {showForm ? (
        <div className="bg-surface rounded-xl border border-outline-variant p-lg max-w-[600px] mx-auto shadow-sm">
          <h2 className="font-headline-md text-headline-md text-primary mb-md">Agregar Cantón</h2>
          <form className="space-y-lg" onSubmit={handleSubmit}>
            <div>
              <label className="block font-label-md text-label-md text-on-surface-variant mb-1" htmlFor="nombre">Nombre del Cantón <span className="text-danger">*</span></label>
            ...
            <div>
              <label className="block font-label-md text-label-md text-on-surface-variant mb-1" htmlFor="provincia">Provincia <span className="text-danger">*</span></label>
              <select id="provincia" required value={provincia} onChange={(e) => setProvincia(e.target.value)}
                className="w-full px-md py-3 border border-outline rounded-lg font-body-md bg-surface-container-lowest input-focus-ring transition-all">
        ...
        <>
          <div className="flex items-center justify-between mb-lg">
            <div>
              <h1 className="font-headline-lg text-headline-lg font-bold text-on-surface">Cantones</h1>
              <p className="font-body-md text-body-md text-on-surface-variant">Total: {cantones.length} registros</p>
            </div>
            <button onClick={() => setShowForm(true)} className="inline-flex items-center gap-md px-lg py-md bg-primary text-on-primary rounded-lg font-label-md font-bold hover:brightness-110 transition-all">
              <span className="material-symbols-outlined">add</span>Nuevo Cantón
            </button>
          </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead className="bg-surface-container-low sticky top-0">
                  <tr>
                    ...
                    <th className="px-lg py-md font-label-md text-label-md text-on-surface-variant border-b border-outline-variant">Provincia</th>
                ...
                <tbody className="divide-y divide-outline-variant">
                  {loading ? (
                    ...
                    <tr><td colSpan="4" className="px-lg py-md text-center text-on-surface-variant font-body-md">No hay cantones registrados</td></tr>
                  ) : cantones.map((c) => (
                    <tr key={c.id} className="hover:bg-surface-container-lowest transition-colors">
                      ...
                      <td className="px-lg py-md font-body-md text-on-surface-variant">{c.provincia?.nombre || c.provincia}</td>
