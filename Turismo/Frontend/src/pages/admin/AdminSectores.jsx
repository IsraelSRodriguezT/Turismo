...
export default function AdminSectores() {
  ...
  const cargarDatos = async () => {
    ...
    setError(null);
    try {
      const [respSectores, respParroquias] = await Promise.all([
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await api.post('/geolocalizacion/sectores/', { nombre, parroquia_id: Number(parroquia) });
      setNombre('');
      setParroquia('');
      setShowForm(false);
      cargarDatos();
    } catch {
      alert('Error al guardar el sector. Verifique que no esté duplicado.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleEliminar = async (id, nombreS) => {
    if (!confirm(`¿Eliminar el sector "${nombreS}"?`)) return;
    try {
      await api.delete(`/geolocalizacion/sectores/${id}/`);
      cargarDatos();
    } catch {
  ...
  return (
    <>
      <nav className="flex items-center gap-xs text-on-surface-variant mb-lg font-label-md text-label-md">
        ...
        <Link to="/admin/sectores" onClick={() => setShowForm(false)} className="hover:text-primary">Gestión de Sectores</Link>
        {showForm && (<><span className="material-symbols-outlined text-[14px]">chevron_right</span><span className="font-bold text-on-surface">Nuevo Sector</span></>)}
      </nav>

      {showForm ? (
        <div className="bg-surface rounded-xl border border-outline-variant p-lg max-w-[600px] mx-auto shadow-sm">
          <h2 className="font-headline-md text-headline-md text-primary mb-md">Agregar Sector</h2>
          <form className="space-y-lg" onSubmit={handleSubmit}>
            <div>
              <label className="block font-label-md text-label-md text-on-surface-variant mb-1" htmlFor="nombre">Nombre del Sector <span className="text-danger">*</span></label>
            ...
            <div>
              <label className="block font-label-md text-label-md text-on-surface-variant mb-1" htmlFor="parroquia">Parroquia <span className="text-danger">*</span></label>
              <select id="parroquia" required value={parroquia} onChange={(e) => setParroquia(e.target.value)}
                className="w-full px-md py-3 border border-outline rounded-lg font-body-md bg-surface-container-lowest input-focus-ring transition-all">
            ...
            <div className="space-y-md">
              <button type="submit" disabled={submitting} className="w-full py-3 bg-primary text-on-primary rounded-lg font-label-md font-bold hover:brightness-110 transition-all disabled:opacity-50">
                {submitting ? 'Guardando...' : 'Guardar Sector'}
              </button>
        ...
        <>
          <div className="flex items-center justify-between mb-lg">
            <div>
              <h1 className="font-headline-lg text-headline-lg font-bold text-on-surface">Sectores</h1>
              <p className="font-body-md text-body-md text-on-surface-variant">Total: {sectores.length} registros</p>
            </div>
            <button onClick={() => setShowForm(true)} className="inline-flex items-center gap-md px-lg py-md bg-primary text-on-primary rounded-lg font-label-md font-bold hover:brightness-110 transition-all">
              <span className="material-symbols-outlined">add</span>Nuevo Sector
            </button>
          </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead className="bg-surface-container-low sticky top-0">
                  <tr>
                    ...
                    <th className="px-lg py-md font-label-md text-label-md text-on-surface-variant border-b border-outline-variant">Parroquia</th>
                ...
                <tbody className="divide-y divide-outline-variant">
                  {loading ? (
                    ...
                    <tr><td colSpan="4" className="px-lg py-md text-center text-on-surface-variant font-body-md">No hay sectores registrados</td></tr>
                  ) : sectores.map((s) => (
                    <tr key={s.id} className="hover:bg-surface-container-lowest transition-colors">
                      <td className="px-lg py-md font-body-md text-on-surface-variant">{s.id}</td>
                      <td className="px-lg py-md font-body-md text-on-surface font-medium">{s.nombre}</td>
                      <td className="px-lg py-md font-body-md text-on-surface-variant">{s.parroquia?.nombre || s.parroquia}</td>
                      <td className="px-lg py-md">
                        <button onClick={() => handleEliminar(s.id, s.nombre)}
