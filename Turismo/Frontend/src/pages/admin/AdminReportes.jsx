...
export default function AdminReportes() {
  ...
  const exportarUsuariosCSV = async () => {
    try {
      ...
      const headers = 'ID,Nickname,Correo,Nombre,Apellido,Estado,Superusuario,Staff\n';
      const rows = usuarios.map(u => 
        `"${u.id}","${u.nickname}","${u.correo}","${u.nombre || ''}","${u.apellido || ''}","${u.is_active ? 'Activo' : 'Inactivo'}","${u.is_superuser}","${u.is_staff}"`
      ).join('\n');
          ...
          <div className="w-full max-w-lg mx-4 bg-surface rounded-xl border border-outline-variant shadow-xl"
            ...
            <form onSubmit={handleSubmit} className="p-lg space-y-lg">
              ...
              <div className="flex gap-md pt-4 border-t border-outline-variant">
                <button type="submit"
                  className="flex-1 py-3 bg-primary text-on-primary rounded-lg font-label-md font-bold hover:brightness-110 transition-all">
                  Generar Reporte
                </button>
                <button type="button" onClick={cerrarForm}
